...
func buildSystemPrompt(sqlEnabled bool) string {
	parts := []string{
		...
		"",
		"How to respond:",
		"  1. For information questions, call `query_database` with a SELECT against the catalog tables. Synthesize a one-or-two-sentence answer from the rows.",
		...
		"  5. Be terse. Spoken responses must be one or two sentences max. Use callsigns (ROOK-1, BOGEY-7) verbatim from the data.",
		"",
